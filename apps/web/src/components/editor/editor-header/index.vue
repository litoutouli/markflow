...
import { Bot, Copy, ImagePlus, Menu, Palette } from 'lucide-vue-next'
import AIAssistantPanel from '@/components/ai/chat-box/AIAssistantPanel.vue'
import AIImageGeneratorPanel from '@/components/ai/image-generator/AIImageGeneratorPanel.vue'
...
import markflowIcon from '@/assets/images/markflow-icon.png'
...
<template>
  <header
    class="header-container h-15 flex flex-nowrap items-center justify-between gap-4 px-5 relative"
  >
    <!-- 桌面端左侧菜单 -->
    <div class="hidden md:flex min-w-0 flex-1 items-center gap-3 overflow-hidden">
      <div class="brand-lockup" aria-label="MarkFlow">
        <img class="brand-mark" :src="markflowIcon" alt="" aria-hidden="true">
        <span class="brand-text">MarkFlow</span>
      </div>
      <Menubar class="menubar min-w-0 shrink overflow-hidden border-0">
    ...
    <div class="header-actions flex shrink-0 items-center gap-2">
      ...
      >
        <Bot class="h-4 w-4 lg:mr-2" />
        <span class="hidden lg:inline">助手</span>
      </Button>
      ...
      >
        <ImagePlus class="h-4 w-4 lg:mr-2" />
        <span class="hidden lg:inline">文生图</span>
      </Button>
  ...
  <AIAssistantPanel v-model:open="uiStore.aiDialogVisible" />
...
.brand-mark {
  ...
  height: 2.25rem;
  object-fit: contain;
  filter: drop-shadow(0 5px 10px rgb(15 23 42 / 0.12));
}
