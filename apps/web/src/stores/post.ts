...
const DEFAULT_POST_TITLE = `MarkFlow 介绍`
const LEGACY_DEFAULT_MARKERS = [
  `# 探索 Markdown 的奇妙世界`,
  `cdn-${`do`}${`ocs`}.oss-cn-shenzhen.aliyuncs.com/gh/${`do`}${`ocs`}/md/images/1648303220922`,
  `### 推荐阅读`,
]
...
export const usePostStore = defineStore(`post`, () => {
  ...
  function migrateLegacyDefaultPost() {
    const firstPost = posts.value[0]

    if (!firstPost)
      return

    const isLegacyDefaultPost = firstPost.title === `内容1`
